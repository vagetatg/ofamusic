...
from src.modules.utils import (
    Filter, SupportButton, get_audio_duration, play_button, sec_to_min
)
...
async def _update_msg_with_thumb(c: Client, msg: types.Message, text: str, thumb: str, button: types.ReplyMarkupInlineKeyboard):
...
async def _handle_single_track(c: Client, msg: types.Message, chat_id: int,
    ...
    if chat_cache.is_active(chat_id):
        ...
        await _update_msg_with_thumb(c, msg, text, thumb, play_button(0, 0))
        return None
    ...
    reply = await _update_msg_with_thumb(c, msg, text, thumb, play_button(0, song.duration))
...
async def _handle_multiple_tracks(c: Client, msg: types.Message, chat_id: int,
    ...
    await edit_text(msg, text, reply_markup=play_button(0, 0))
...
async def _handle_recommendations(c: Client, msg: types.Message, wrapper: MusicServiceWrapper):
...
async def _handle_telegram_file(c: Client, msg: types.Message, reply: types.Message,
...
async def _handle_text_search(c: Client, msg: types.Message, chat_id: int,
    ...
    if play_type == 0:
        url = _get_platform_url(platform, search.tracks[0].id)
